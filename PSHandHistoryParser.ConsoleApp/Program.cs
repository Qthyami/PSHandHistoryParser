...
namespace PSHandHistoryParser.ConsoleApp
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            var result = PSHandParser.ParseHand(hand);
            Console.WriteLine($"Hand #{result.handNumber}");
            foreach (var player in result.playerInfos)
            {
                Console.WriteLine($"Seat {player.seatNumber}: {player.nickName}");
            }
        }
    }
}
